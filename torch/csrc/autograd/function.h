...
struct Function : std::enable_shared_from_this<Function> {
 public:
  ...
  /// Evaluate the function on the given inputs and return the result of the
  /// function call.
  ...
  void swap_next_edges(edge_list& new_edges) {
  ...
  /// `PyFunction`s are not managed by shared_ptrs by default, but are bound to
  /// the lifetime of their Python object instead.
  virtual std::shared_ptr<Function> get_shared_ptr() {
    return shared_from_this();
  }

  /// Return the name of the dynamic type of the function, for debugging.
  virtual std::string name();

  /// Return true if the particular output edge is active, and that particular
  ...
  /// Return true if any of the output edges in any of the ranges are active.
  ...
  /// Return the `PyObject` stored for this `Function` (for Python
  /// interaction).
  PyObject* pyobj() const noexcept {
    return pyobj_;
  }

  /// Set the `PyObject` stored for this `Function` (for Python interaction).
  ...
  /// Release saved variables if the operation won't be reused.
  ...
  /// Return true if this function is traceable. An op is traceable if all
  ...
  /// Return `Variable`s saved by this `Function`.
