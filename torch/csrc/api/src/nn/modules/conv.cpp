...
ConvImpl<D, Derived>::ConvImpl(ConvOptions<D> options)
  ...
  if (options_.with_bias_) {
    bias_ = this->add(
        Var(at::CPU(at::kFloat).empty(options_.output_channels_)), "bias");
  }
...
variable_list Conv1dImpl::forward(variable_list input) {
...
variable_list Conv2dImpl::forward(variable_list input) {
...
variable_list Conv3dImpl::forward(variable_list input) {
