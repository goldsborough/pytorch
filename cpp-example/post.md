...
```sh
Forward: 78.837 us | Backward 118.974 us
```

and with our new C++ version:

```sh
Forward: 46.575 us | Backward 106.598 us
```

We can already see a significant speedup for the forward function. For the
backward function a speedup is visible, albeit not major one. The backward pass
I wrote above was not particularly optimized and could definitely be improved.
Note also that the PyTorch automatic differentiation engine can automatically
parallelize computation graphs, and may also use a more efficient flow of
operations overall. Nevertheless, this is a good start.
