...
void GraphTask::init_to_execute(Function& graph_root, const edge_list& outputs) {
  ...
  int output_idx = 0;
  for (auto & next_edge : outputs) {
    Function *output = next_edge.function.get();
    auto & info = exec_info[output];
    if (!info.captures)
      info.captures.reset(new std::vector<ExecInfo::Capture>());
    info.captures->emplace_back(next_edge.input_nr, output_idx++);
  }
  ...
  //   return is_needed[fn]
  struct Frame {
    Frame (Function *fn) : fn(fn), next_next_edge(0) {}
    Function *fn;
    std::size_t next_next_edge;

    Function* get_next_edge() {
      const auto & next = fn->next_edges();
      auto num_next = next.size();
      while (next_next_edge < num_next) {
        auto fn = next[next_next_edge++].function.get();
        if (fn) return fn;
      }
      return nullptr;
    }
  ...
  for (const auto & input : graph_root.next_edges()) {
    ...
    while (!stack.empty()) {
      ...
      if (Function *next_edge = frame.get_next_edge()) {
        if (/* bool unseen = */ seen.emplace(next_edge).second) {
          stack.emplace_back(next_edge);
          continue; // recurse
        }
      } else {
