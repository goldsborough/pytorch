...
bool pointer_equal(Tensor key, Tensor second) {
  return key.data<float>() == second.data<float>();
}
...
TEST_CASE("module/zero-grad") {
  ...
  backward(loss);
  for (auto& parameter : module->parameters2()) {
...
TEST_CASE("module/clone") {
  ...
  //   auto m1param = module->parameters2();
  //   auto m2param = module2->parameters2();
  ...
  SECTION("Cloning preserves external references") {
    ...
    REQUIRE(pointer_equal(module->weight, module->parameters2()["weight"]));
    REQUIRE(module->weight.allclose(module->parameters2()["weight"]));
    ...
    REQUIRE(pointer_equal(module2->weight, module2->parameters2()["weight"]));
    REQUIRE(module2->weight.allclose(module2->parameters2()["weight"]));
    REQUIRE(module2->weight.allclose(module->weight));
    REQUIRE(!pointer_equal(module2->weight, module->parameters2()["weight"]));
  }
  ...
  SECTION("Cloning copies the values of variables of submodules") {
    ...
    REQUIRE(
        pointer_equal(b->module->weight, b->module->parameters2()["weight"]));
    REQUIRE(b->module->parameters2()["weight"].allclose(a->module->weight));
...
TEST_CASE("module/parameters") {
  ...
  SECTION("has correct number of parameters") {
    REQUIRE(module.parameters2().count() == 3);
  }

  SECTION("contains parameters with the correct name") {
    auto parameters = module.parameters2();
